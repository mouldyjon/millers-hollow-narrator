...
const roleActions: Record<string, ActionStep[]> = {
  ...
  'white-werewolf': [
    { text: 'Wake the White Werewolf alone (every other night)', important: true },
  ...
  'witch': [
    { text: 'Show the Witch who the werewolves chose as victim', important: true },
  ...
  'fox': [
    { text: 'Ask the Fox to point at a player and their two neighbours', important: true },
...
export const RoleActionGuide = ({ roleId, onStepComplete }: RoleActionGuideProps) => {
  const actions = roleActions[roleId] || [];

  if (actions.length === 0) {
    return (
      <div className="bg-slate-700 rounded-lg p-4 mt-4">
        <p className="text-sm text-slate-300">No special actions required for this role during night phase.</p>
      </div>
    );
  }
      ...
      <div className="space-y-2">
        {actions.map((action, index) => (
          ...
          >
            ...
            <p className={`text-sm ${action.completed ? 'line-through text-slate-500' : 'text-slate-200'}`}>
              {action.text}
            </p>
          </div>
        ))}
